...
class OpenFoodFactsServer {
  ...
  private async searchProducts(args: any) {
    ...
    const response = await axios.get<OpenFoodFactsSearchResponse>(url, { params });
  ...
  private async getProductByBarcode(args: any) {
    const { barcode } = args;
  ...
  private async getProductsByCategory(args: any) {
  ...
  private async getProductsByBrand(args: any) {
  ...
  private async getNutritionFacts(args: any) {
    const { barcode } = args;
  ...
  private async compareProducts(args: any) {
    const { barcodes } = args;
  ...
  private async getAllergenInfo(args: any) {
    const { barcode } = args;
  ...
  private async searchByNutriments(args: any) {
    const { 
      max_fat, 
      max_sugar, 
      max_salt, 
    ...
    if (max_fat !== undefined) searchParams.append('nutriment_fat_100g_max', max_fat.toString());
    if (max_sugar !== undefined) searchParams.append('nutriment_sugars_100g_max', max_sugar.toString());
    if (max_salt !== undefined) searchParams.append('nutriment_salt_100g_max', max_salt.toString());
    if (min_fiber !== undefined) searchParams.append('nutriment_fiber_100g_min', min_fiber.toString());
    if (min_protein !== undefined) searchParams.append('nutriment_proteins_100g_min', min_protein.toString());
    if (nutriscore_grade) searchParams.append('nutriscore_grade', nutriscore_grade.toLowerCase());

    const url = `${this.baseUrl}/search.json?${searchParams.toString()}`;
    const response = await axios.get<OpenFoodFactsSearchResponse>(url);
  ...
  private formatProductSummary(product: OpenFoodFactsProduct) {
    return {
      barcode: product.code,
  ...
  private formatProductDetails(product: OpenFoodFactsProduct) {
    return {
      barcode: product.code,
      name: product.product_name || 'Unknown',
      brands: product.brands || 'Unknown',
